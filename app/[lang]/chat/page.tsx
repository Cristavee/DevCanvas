...
import {
  ...
  MessageSquare, ChevronRight, Code2, AtSign, Bell, Pin
} from "lucide-react";

type MessageType = { id: number; from: string; fromMe: boolean; text: string; time: string; avatar: string; type?: 'code' | 'text' };
...
const MESSAGES_MAP: Record<number, MessageType[]> = {
  1: [
    ...
    { id: 4, from: 'You', fromMe: true, text: "Yes! Just prefers-color-scheme media query and adjust the gradient stops. I can share the full version.", time: '10:08 AM', avatar: 'U', type: 'code' },
...
export default function ChatPage() {
  ...
  const handleSend = () => {
    ...
    setMessages(prev => [...prev, { id: prev.length + 1, from: 'You', fromMe: true, text: input.trim(), time: formatTime(new Date()), avatar: 'U' }]);
    setInput('');
  };
  ...
  return (
    <div className="flex h-[calc(100vh-100px)] lg:h-[calc(100vh-64px)] -mt-6 -mx-4 lg:-mx-8 overflow-hidden rounded-none lg:rounded-xl border border-border bg-card">
      ...
      <div className={`${showSidebar ? 'flex' : 'hidden lg:flex'} flex-col w-full lg:w-64 xl:w-72 border-r border-border flex-shrink-0`}
        ...
        <div className="flex-1 overflow-y-auto py-2">
          {[{ label: 'Direct Messages', items: dms }, { label: 'Group Chats', items: groups }, { label: 'Channels', items: channels }].map(({ label, items }) => items.length > 0 && (
                ...
                <button key={chat.id} onClick={() => handleSelectChat(chat)}
                  ...
                  <div className="relative flex-shrink-0">
                    <div className="w-8 h-8 rounded-lg flex items-center justify-center text-xs font-bold text-black"
                      style={{ background: chat.color, opacity: chat.type === 'channel' ? 1 : 1 }}>
            ...
            <div className="flex items-center justify-between px-4 py-3 border-b border-border flex-shrink-0">
              <div className="flex items-center gap-3">
                ...
                <div className="relative">
                  ...
                  {activeChat.type === 'dm' && <div className={`absolute -bottom-0.5 -right-0.5 w-2.5 h-2.5 rounded-full border-2 border-card ${activeChat.online ? 'status-online' : 'status-offline'}`} />}
                </div>
                <div>
                  ...
                  <div key={msg.id} className={`flex items-end gap-2.5 ${msg.fromMe ? 'flex-row-reverse' : ''} ${grouped ? 'mt-0.5' : 'mt-4'}`}>
                    ...
                    <div className={`flex flex-col ${msg.fromMe ? 'items-end' : 'items-start'} max-w-[72%]`}>
                      ...
                      <div className={`px-3.5 py-2 rounded-2xl text-sm leading-relaxed ${msg.fromMe ? 'rounded-br-sm' : 'rounded-bl-sm'}`}
                        style={msg.fromMe ? { background: 'var(--neon-green)', color: 'hsl(var(--background))' } : { background: 'hsl(var(--muted))' }}>
                        {msg.text}
                      </div>
